...
class Shors {
  ...
  void qft(std::vector<int> qubits, bool inverse = false) {
    ...
    if (inverse)
      std::reverse(qubitRange.begin(), qubitRange.end());
    for (int j : qubitRange) {
      std::vector<int> neighbours(j - std::max(0, j - (int)qubits.size() + 1));
      std::iota(neighbours.begin(), neighbours.end(),
                std::max(0, j - (int)qubits.size() + 1));
      if (inverse) {
  ...
  void ccphiAdd(int control1, int control2, int q, int64_t a,
                bool inverse = false) {
    auto angle = angles(a);
    for (int i = 0; i < n + 1; i++) {
      QGate *u;
  ...
  void cMulModN(int control, int q, int aux, int64_t a) {
    ...
    qft(qs);
    for (int i = 0; i < n; i++)
      ccphiAddModN(q + i, control, aux, aux + n + 1, (1 << i) * (a % N));
    qft(qs, true);
    for (int i = 0; i < n; i++) {
    ...
    qft(qs);
    for (int i = 0; i < n; i++)
      ccphiAddModNInv(q + i, control, aux, aux + n + 1, (1 << i) * (aInv % N));
    qft(qs, true);
  }
...
public:
  ...
  std::vector<int64_t> solve() {
    ...
    if (r % 2 == 1 || ((int64_t)pow(a, r / 2)) % N == N - 1)
      return solve();
